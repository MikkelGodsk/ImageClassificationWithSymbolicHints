...
def evaluate_model(model: pl.LightningModule, dataloader: torch.utils.data.DataLoader, model_name: str):
    trainer = pl.Trainer(
...
def experiment1(
    ...
    text_clf = get_text_classifier(train_text_ds=train_text_ds, load_bert=load_bert, dataset_=dataset_)
    ...
    if RUN_EXPERIMENT_1:
        ...
        evaluate_model(text_clf, text_dataloader(bimodal_val_ds.text_ds, shuffle=False), "Uncalibrated {:s} based classifier".format(text_clf_name))
        eval_text_end = time()
        print_time(0, eval_text_end-eval_text_start)
        eval_img_start = time()
        evaluate_model(img_clf, img_dataloader(bimodal_val_ds.image_ds, shuffle=False), "Uncalibrated {:s} classifier".format(img_clf_name))
        eval_img_end = time()
...
def experiment2(
    ...
    """    
    fusion_model: MultimodalModel.NaiveBayesFusion = MultimodalModel.NaiveBayesFusion(img_clf, text_clf)
    if RUN_EXPERIMENT_2:
        ...
        evaluate_model(fusion_model, bimodal_dataloader(bimodal_val_ds, shuffle=False), "Addition-based fusion of uncalibrated {:s} and uncalibrated {:s}".format(img_clf_name, text_clf_name))
        eval_combined_end = time()
        print_time(prep_combined_end-prep_combined_start, eval_combined_end-eval_combined_start)
        fusion_model.cuda()
        LitModel.reliability_plot(fusion_model, bimodal_dataloader(bimodal_val_ds, shuffle=False), "Uncalibrated fusion model for {:s}".format(dataset))
...
def experiment3(
    ...
    img_clf: img_clf_t,
    bimodal_val_ds: BimodalDS
) -> Tuple[LitModel.CalibratedLitModel, LitModel.CalibratedLitModel, MultimodalModel.NaiveBayesFusion]:
    """
    ...
    calibrated_text_clf: LitModel.CalibratedLitModel = LitModel.calibrate(
        text_clf.cuda(), 
        text_dataloader(bimodal_val_ds.text_ds, shuffle=False),    # Large batch size is preferred for calibration
    ...
    prep_img_end = time()
    calibrated_fusion_model: MultimodalModel.NaiveBayesFusion = MultimodalModel.NaiveBayesFusion(
        calibrated_img_clf, calibrated_text_clf)
    calibrated_fusion_model.estimate_prior_class_probs(bimodal_val_ds, n_classes=num_classes)
    prep_combined_end = time()
    ...
    eval_text_start = time()
    evaluate_model(calibrated_text_clf, text_dataloader(bimodal_val_ds.text_ds, shuffle=False), "Calibrated {:s} model".format(text_clf_name))
    eval_text_end = time()
    print_time(prep_text_end-prep_text_start, eval_text_end-eval_text_start)
    
    eval_img_start = time()
    evaluate_model(calibrated_img_clf, img_dataloader(bimodal_val_ds.image_ds, shuffle=False), "Calibrated {:s} model".format(img_clf_name))
    eval_img_end = time()
    print_time(prep_img_end-prep_img_start, eval_img_end-eval_img_start)
    
    eval_combined_start = time()
    evaluate_model(calibrated_fusion_model, bimodal_dataloader(bimodal_val_ds, shuffle=False), "Calibrated fusion model")
    eval_combined_end = time()
    print_time(prep_combined_end-prep_combined_start, eval_combined_end-eval_combined_start)
    
    calibrated_fusion_model.cuda()
    LitModel.reliability_plot(calibrated_fusion_model, bimodal_dataloader(bimodal_val_ds, shuffle=False), "Calibrated fusion model for {:s}".format(dataset))  # Reliability plots for the constituent classifiers are made in the call to `LitModel.calibrate`
...
def experiment4a(
    ...
    svm_fusion = MultimodalModel.SVMBasedFusion(img_clf, text_clf, regularization_strength=regularization_strength, n_jobs=6)
    svm_fusion.cuda()
    prep_svm_start = time()
    svm_fusion.fit_svm(
    ...
    svm_fusion.calibrate_svm(
        bimodal_dataloader(val_ds, shuffle=True),
        verbose=DEBUG
    )
    prep_svm_end = time()
    eval_svm_start = time()
    evaluate_model(svm_fusion, bimodal_dataloader(val_ds, shuffle=False), "SVM-based fusion")
    eval_svm_end = time()
...
def experiment4b(
    ...
    """
    params = {
        'lr': 0.0003172298313831537,
        'top_l2_rate': 2.723639829156825e-05
    }
    fusion_model = MultimodalModel.LogisticRegressionBasedFusion(img_clf, text_clf, out_features=num_classes, **params)
    fusion_model.cuda()
    
    logger = pl.loggers.TensorBoardLogger(save_dir="lightning_logs", name="Experiments", version="log_reg_fusion_model_{:s}".format(dataset))
    ...
    evaluate_model(fusion_model, bimodal_dataloader(val_ds, shuffle=False), "Logistic regression fusion model")
    fusion_model.cuda()
    LitModel.reliability_plot(fusion_model, bimodal_dataloader(val_ds, shuffle=False), "Logistic regression fusion model for {:s}".format(dataset))
    return fusion_model
...
def experiment5(
    ...
    logger = pl.loggers.TensorBoardLogger(save_dir="~/bscproj/CMPlaces/lightning_logs", name="Experiments", version="Fine-tuned_fusion_model_{:s}".format(dataset))
    
    prep_combined_start = time()
    fusion_model.estimate_prior_class_probs(train_ds, n_classes=num_classes, precomputed_log_probs=train_log_prior_probs)
    fusion_model.cuda()
    ...
    fusion_model.estimate_prior_class_probs(val_ds, n_classes=num_classes, precomputed_log_probs=val_log_prior_probs)
    prep_combined_end = time()
    
    eval_combined_start = time()
    fusion_model.cuda()
    top_1_acc = evaluate_model(fusion_model, bimodal_dataloader(val_ds, shuffle=False), "Fine-tuned fusion model")
    eval_combined_end = time()
    print_time(prep_combined_end-prep_combined_start, eval_combined_end-eval_combined_start)
    fusion_model.cuda()
    if make_plots:
        LitModel.reliability_plot(fusion_model, bimodal_dataloader(val_ds, shuffle=False), "Fine-tuned fusion model for {:s}".format(dataset))
...
def experiment6(
    ...
    eval_img_start = time()
    evaluate_model(img_clf, img_dataloader(img_ds, shuffle=False), "Recovered (fine-tuned) {:s} model".format(img_clf_name))
    eval_img_end = time()
    print_time(0, eval_img_end-eval_img_start)
    eval_text_start = time()
    evaluate_model(text_clf, text_dataloader(text_ds, shuffle=False), "Recovered (fine-tuned) {:s} model".format(text_clf_name))
    eval_text_end = time()
    print_time(0, eval_text_end-eval_text_start)
    ...
    LitModel.reliability_plot(img_clf, img_dataloader(img_ds, shuffle=False), "Recovered (fine-tuned) {:s} model for {:s}".format(img_clf_name, dataset))
    LitModel.reliability_plot(text_clf, text_dataloader(text_ds, shuffle=False), "Recovered (fine-tuned) {:s} model for {:s}".format(text_clf_name, dataset))
    
    

def main():
    print("\n\033[1m\033[34mUsing dataset: {:s}\033[0m".format(dataset))   #Formatting codes: \033[Xm where X is an integer
    ...
    # Experiment 1
    if RUN_EXPERIMENT_1 or RUN_EXPERIMENT_2 or RUN_EXPERIMENT_3 or RUN_EXPERIMENT_4 or RUN_EXPERIMENT_5 or RUN_EXPERIMENT_6:
    ...
    if (RUN_EXPERIMENT_4 or RUN_EXPERIMENT_5 or RUN_EXPERIMENT_6) and not DEBUG:
        train_img_ds: ImageModalityDS = load_img_ds('train')  # This call will take a long time... (Approx. 40m)
        bimodal_train_ds: BimodalDS = BimodalDS(image_ds=train_img_ds, text_ds=train_text_ds)
    ...
    if RUN_EXPERIMENT_6:
        ...
        experiment6(
            fusion_model = fine_tuned_fusion_model,
            val_ds = bimodal_val_ds
        )
