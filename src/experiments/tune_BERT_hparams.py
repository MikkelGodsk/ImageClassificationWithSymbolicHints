...
def tune_hparams(
    ...
    train_ds, val_split = torch.utils.data.random_split(
        full_train_ds,
        [t, len(full_train_ds) - t],
...
def make_params_v1(trial):
    ...
    params = {
        ...
        "top_dense_layer_units": [
            trial.suggest_int("n_units_l{}".format(i), 128, 256 + 128, log=True)
...
def make_params_v3(trial):
    params = {
        "learning_rate": trial.suggest_loguniform(
            "learning_rate", 5e-7, 1e-1
        ),  # trial.suggest_loguniform("learning_rate", 5e-4, 1e-2),
        "adam_epsilon": 1e-8,
        "weight_decay": 0,  # trial.suggest_loguniform("weight_decay", 1e-10, 1e-1),
        "dropout_rate": trial.suggest_float("dropout_rate", 0, 0.5),
        "top_dense_layer_units": [],
