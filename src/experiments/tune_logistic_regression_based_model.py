...
def objective(
    ...
    trainer = pl.Trainer(
        ...
        callbacks=[optuna.integration.PyTorchLightningPruningCallback(trial, monitor="hp_validation_top_1_acc")],
    )
    fusion_model.cuda()
...
def hyperparam_tuning(
    ...
    logger_fn = lambda: pl.loggers.TensorBoardLogger(save_dir="~/bscproj/CMPlaces/lightning_logs", name="Logistic_regression_model_hparam_search")
...
def tune_hparams(make_params: typing.Callable[[optuna.trial.Trial], typing.Dict[str, typing.Union[float, int, typing.List[int]]]], n_trials: int = 50, timeout: int = 86100):
    ...
    return hyperparam_tuning(bimodal_dataloader(train_split), bimodal_dataloader(val_split), make_params, n_trials=n_trials, timeout=timeout, max_epochs=1)
