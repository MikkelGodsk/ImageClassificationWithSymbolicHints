...
@click.option(
    '--debug',
    default=1,
    help="Whether to use the validation set instead"
)
def main(*args, **kwargs):
    ...
    if not debug:
        ...
        train = load_pickle(os.path.join(ds_dir, dataset_name, 'val.pkl'))
    ...
    top5 = top_k_accuracy_score(val['y'], y_probs, k=1)
    print("Top 1 accuracy: {:f}%".format(top1*100))
    print("Top 5 accuracy: {:f}%".format(top5*100))
