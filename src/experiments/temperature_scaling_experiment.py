...
if not val_split:
    ...
    bimodal_val_ds: torch.utils.data.DataLoader = bimodal_dataloader(
        BimodalDS(image_ds=val_img_ds, text_ds=val_text_ds), batch_size=1
    )
else:
    ...
    bimodal_val_ds: torch.utils.data.DataLoader = bimodal_dataloader(
        BimodalDS(image_ds=val_img_ds, text_ds=val_text_ds), batch_size=1, num_workers=0
    )
...
try:
    if dataset.lower() == "imagenet":
        ...
        log_prior_class_probs = torch.Tensor(
            np.loadtxt("cmplaces_val_prior.txt")
        ).unsqueeze(0)
except FileNotFoundError:
    print(
        "Class priors must be computed beforehand. Run the file `compute_class_priors.py`."
    )
...
with torch.no_grad():
    ...
    for x, y in bimodal_val_ds:
        img, txt = x
        logits_img.append(
            img_clf.forward_no_softmax(img).detach().cpu()
        )  # Seems there could be a memory leak...
...
print(
    "Image clf accuracy: {:.2f}".format(
        torchmetrics.functional.accuracy(logits_img, labels)
    ),
    flush=True,
)
print(
    "Text clf accuracy: {:.2f}".format(
        torchmetrics.functional.accuracy(logits_txt, labels)
    ),
    flush=True,
)
...
for i, img_temp in enumerate(temperatures):  # Image classifier temperatures along rows
    for j, txt_temp in enumerate(
        temperatures
    ):  # Text classifier temperatures along columns
        z = (
            logits_img * img_temp + logits_txt * txt_temp - log_prior_class_probs
        )  # Log prior is the culprit.
        z.mul(mask)
