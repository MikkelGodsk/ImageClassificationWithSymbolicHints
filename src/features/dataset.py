...
def correct_labeling(x):
    ...
    if labels_translation_dict is None:
        ...
        dir_labels = sorted(
            os.listdir(os.path.join(ds_dir, "val"))
...
def load_img_ds(filename):
    ...
    return torchvision.datasets.ImageFolder(
        filename, transform=transform, target_transform=correct_labeling
    )
...
###############
def split_img_ds(
    img_ds: ImageModalityDS, train_val_split: float
) -> Tuple[ImageModalityDS, ImageModalityDS]:
...
def split_text_ds(
    text_ds: TextModalityDS, train_val_split: float
) -> Tuple[TextModalityDS, TextModalityDS]:
...
def img_dataloader(
    ds: ImageModalityDS, shuffle=True, batch_size=32, **kwargs
...
def text_dataloader(
    ds: TextModalityDS, shuffle=True, batch_size=32, **kwargs
) -> torch.utils.data.DataLoader:
    return torch.utils.data.DataLoader(
...
def bimodal_dataloader(
    ds: BimodalDS, shuffle=True, batch_size=32, **kwargs
) -> torch.utils.data.DataLoader:
