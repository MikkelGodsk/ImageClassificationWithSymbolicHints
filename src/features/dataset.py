...
def correct_labeling(x):
    ...
    if labels_translation_dict is None:
        ...
        dir_labels = sorted(os.listdir(os.path.join(ds_dir, 'val')))   # Has been tested to be equal to ordering of ds.class_to_idx.keys()
        dir_labels = dict(zip(dir_labels, range(0, len(dir_labels))))
        labels_translation_dict = {}
        with open(os.path.join(ds_dir, 'categories.txt'), 'r') as f_obj:
...
def load_text_dict(filename):
    with open(filename, 'r') as f_obj:
        txt_dict = json.load(f_obj)
    return txt_dict


def text_dict_to_ds_list(text_dict):
    classes = sorted(text_dict.keys())
    X = []
    y = []
    
    for i, c in enumerate(classes):
        corrected_i = correct_labeling(i)   # Transform to correct indexing.
...
def build_class_to_idx(text_dict):
    classes = sorted(text_dict.keys())
    class_to_idx = {}
    for i, c in enumerate(classes):
        class_to_idx[c] = correct_labeling(i)
        
    return class_to_idx


def build_dataset_dict(text_dict, class_to_idx):
    ds_dict = {}
...
## Dataset loaders ##
#####################
def load_img_ds(filename):
    if dataset == 'imagenet':     # Transform from example https://pytorch.org/vision/stable/models.html
    ...
    elif dataset == 'cmplaces':   # Transform from https://github.com/keras-team/keras-applications/blob/master/keras_applications/imagenet_utils.py
        ...
        transform = torchvision.transforms.Compose([
            ...
            lambda x: x*255,                      # ToTensor scales from 0-255 to 0-1. Here I scale back.
            torchvision.transforms.Normalize(mean, torch.Tensor([1.0,1.0,1.0]), inplace=False)
        ])
    filename = os.path.join(ds_dir, filename)
    return torchvision.datasets.ImageFolder(
            filename,
            transform=transform,
            target_transform=correct_labeling
        )


def load_text_ds(filename):
    filename = os.path.join(ds_dir, filename)
    return TextModalityDS(
        root=filename,
    )
...
class TextModalityDS(torch.utils.data.Dataset):
    ...
    """
    def __init__(self, root: str = None, ds_dict: Dict[int, List[str]] = None, transform=None, target_transform=None):
        self._data_root = root
        if root is not None:
            text_dict = load_text_dict(root)
            self._X, self._y = text_dict_to_ds_list(text_dict)
            self._class_to_idx = build_class_to_idx(text_dict)
            self._dataset_dict = build_dataset_dict(text_dict, self._class_to_idx)
    ...
    def get_text_for_class(self, class_id):   # Needs to live outside of the class....
        ...
        return descriptions[torch.randint(0,len(descriptions),(1,))]    # Pick random description for the queried class
...
class BimodalDS(torch.utils.data.Dataset):
    ...
    """
    def __init__(self, image_ds: ImageModalityDS, text_ds: TextModalityDS):
    ...
    @property
    def image_ds(self) -> ImageModalityDS:
        return self._image_ds
    
    @property
    def text_ds(self) -> TextModalityDS:
...
## Utilities ##
###############
def split_img_ds(img_ds: ImageModalityDS, train_val_split: float) -> Tuple[ImageModalityDS, ImageModalityDS]:
    t = int(len(img_ds)*train_val_split)
    train_split, val_split = torch.utils.data.random_split(img_ds, [t, len(img_ds)-t], generator=torch.Generator().manual_seed(42))
    return train_split, val_split


def split_text_ds(text_ds: TextModalityDS, train_val_split: float) -> Tuple[TextModalityDS, TextModalityDS]:
    ...
    for y, Xs in ds_dict.items():
        ...
        if len(Xs) > 1:
            ...
            train_ds_dict[y], val_ds_dict[y] = torch.utils.data.random_split(Xs, split_lengths, generator=torch.Generator().manual_seed(42))
        elif len(Xs) == 1:
            X = Xs[0]
            s = X.split(' ')
            train_ds_dict[y], val_ds_dict[y] = ' '.join(s[:int(len(s)*0.5)]), ' '.join(s[int(len(s)*0.5):])
        
        assert len(train_ds_dict[y]) and len(val_ds_dict[y]), "Class {:d} has {:d} training instances and {:d} validation instances in the text modality (it requires at least one sentence that can be split)".format(y, len(train_ds_dict[y]), len(val_ds_dict[y]))
...
def img_dataloader(ds: ImageModalityDS, shuffle=True, batch_size=32, **kwargs) -> torch.utils.data.DataLoader:
...
def text_dataloader(ds: TextModalityDS, shuffle=True, batch_size=32, **kwargs) -> torch.utils.data.DataLoader:
    return torch.utils.data.DataLoader(
        ds,
        shuffle=shuffle,
        batch_size=batch_size,
...
def bimodal_dataloader(ds: BimodalDS, shuffle=True, batch_size=32, **kwargs) -> torch.utils.data.DataLoader:
...
###############
def test_text():
    ds = TextModalityDS('val_text.json')
...
def test_split_text():
    ds = TextModalityDS('val_text.json')
    train, val = split_text_ds(ds, train_val_split=0.8)
    for i in range(len(ds.classes)):
        ...
        assert len(X_t) + len(X_v) == len(ds.ds_dict[i])   # All class i elements are in either X_val[i] or X_train[i]
