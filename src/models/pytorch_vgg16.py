...
class KitModel(nn.Module):
    def __init__(self, weight_file):
        ...
        self.fc6_1 = self.__dense(
            name="fc6_1", in_features=25088, out_features=4096, bias=True
        )
        self.fc7_1 = self.__dense(
            name="fc7_1", in_features=4096, out_features=4096, bias=True
        )
        self.fc8_1 = self.__dense(
            name="fc8_1", in_features=4096, out_features=205, bias=True
        )
    ...
    def forward_no_top(self, x):
        ...
        relu6 = F.relu(fc6_1)
        drop6 = F.dropout(
            input=relu6, p=0.5, training=self.training
        )  # , inplace = True)
        fc7_0 = drop6.view(drop6.size(0), -1)
        fc7_1 = self.fc7_1(fc7_0)
        relu7 = F.relu(fc7_1)
        drop7 = F.dropout(
            input=relu7, p=0.5, training=self.training
        )  # , inplace = True)  # Inplace operations messes up gradient computations, it seems.
        fc8_0 = drop7.view(drop7.size(0), -1)
        return fc8_0

    def forward_no_softmax(self, x):
        fc8_0 = self.forward_no_top(x)
        return self.fc8_1(fc8_0)

    def forward(self, x):
        fc8_1 = self.forward_no_softmax(x)
    ...
    def __conv(dim, name, **kwargs):
        ...
        if "bias" in _weights_dict[name]:
            layer.state_dict()["bias"].copy_(
                torch.from_numpy(_weights_dict[name]["bias"]).flatten()
            )  # Added .flatten
        return layer
