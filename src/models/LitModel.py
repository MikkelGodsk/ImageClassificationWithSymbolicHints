...
from reliability import reliability_diagram  # From GitHub. Download https://github.com/hollance/reliability-diagrams and save as reliability.py
...
class LitModel(pl.LightningModule, ABC):
    ...
    def __init__(self, n_bins=25, **kwargs):
        ...
        self.expected_calib_error = torchmetrics.CalibrationError(n_bins=n_bins, norm='l1', compute_on_cpu=True)
        self.maximum_calib_error = torchmetrics.CalibrationError(n_bins=n_bins, norm='max', compute_on_cpu=True)
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log("hp_train_top_1_acc", self.train_top_1_acc, on_epoch=True, on_step=False)
        self.log("hp_train_top_5_acc", self.train_top_5_acc, on_epoch=True, on_step=False)
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log("hp_validation_top_1_acc", self.val_top_1_acc, on_step=False, on_epoch=True)
        self.log("hp_validation_top_5_acc", self.val_top_5_acc, on_step=False, on_epoch=True)
...
class CalibratedLitModel(LitModel):
    ...
    """
    def __init__(self, clf: LitModel, init_temperature=1.0, n_bins=15, learning_rate=0.01, max_iter=50):
        super().__init__(n_bins=n_bins)
        self.clf = clf
        self.temperature = torch.nn.Parameter(torch.ones(1)*init_temperature, requires_grad=True)
        self.softmax = torch.nn.Softmax(dim=-1)
        self.clf.eval()
    ...
    def _configure_optim_train(self):
        return torch.optim.LBFGS([self.temperature], lr=self.learning_rate, max_iter=self.max_iter)
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.maximum_calib_error(z, y)
        self.log("hp_train_loss", loss.item(), on_step=True, on_epoch=False)
        self.log("hp_train_ece", self.expected_calib_error, on_step=False, on_epoch=True)
        self.log("hp_train_mce", self.maximum_calib_error, on_step=False, on_epoch=True)
        self.log("hp_temperature", self.temperature.item(), on_step=True, on_epoch=False)
        return loss
...
def reliability_values(clf: Union[LitModel, CalibratedLitModel], ds: torch.utils.data.DataLoader):
    clf.eval()
    ...
    y_confs = y_probs[torch.arange(len(y_preds)), y_preds]  # Estimated probability of true class.

    return {'true_labels': y_trues, 'pred_labels': y_preds, 'confidences': y_confs}, y_probs


def reliability_plot(clf: Union[LitModel, CalibratedLitModel], ds: torch.utils.data.DataLoader, fig_name: str):
    """
...
def calibrate(clf: Union[LitModel, CalibratedLitModel], val_ds: torch.utils.data.DataLoader, model_name: str, n_bins: int = 15, save_dir="~/bscproj/CMPlaces", learning_rate=0.01, max_iter=50) -> CalibratedLitModel:
    """
    ...
    if isinstance(clf, CalibratedLitModel):
        ...
        calibrated_clf = CalibratedLitModel(clf, n_bins=n_bins, learning_rate=learning_rate, max_iter=max_iter).cuda()
    calibrated_clf.calibrate(val_ds)
    
    # Second reliability plot:
    reliability_plot(calibrated_clf, val_ds, 'reliability_calibrated_' + model_name + '.svg')
    
    print("\n\nFinal temperature of calibrated {:s}: {:.16f}\n\n".format(model_name, calibrated_clf.temperature.detach().cpu().numpy()[0]))
    
    return calibrated_clf

def manual_temperature_search_plot(clf: LitModel, ds: torch.utils.data.DataLoader, temperatures = None, model_name=None, n_bins=15, calibration_lr=0.01, calibration_max_iter=50):
    import numpy as np

    temperatures = temperatures if temperatures is not None else np.arange(0.5,2.5,0.005)
    
    calibrated_clf = CalibratedLitModel(clf, n_bins=n_bins, learning_rate=calibration_lr, max_iter=calibration_max_iter)
    ...
    for t in temperatures:
        ...
        ece.append(torchmetrics.functional.calibration_error(logits/t, labels, n_bins=n_bins, norm='l1'))
        mce.append(torchmetrics.functional.calibration_error(logits/t, labels, n_bins=n_bins, norm='max'))
    ...
    if model_name is not None:
        import matplotlib.pyplot as plt
        plt.plot(temperatures, nll, temperatures, avg_conf, temperatures, ece, temperatures, mce)
        plt.plot(np.hstack((optim_t, optim_t)), np.hstack((0, np.max(nll))), '--', alpha=0.7)
        plt.title("Calibration metrics vs temperatures for {:s}".format(model_name))
        plt.legend(["NLL", "Avg. conf", "ECE", "MCE", "Estimated optimal T"])
        if not os.path.isdir("reliability_plots"): 
            os.mkdir("reliability_plots")
        plt.savefig(os.path.join("reliability_plots", "Temperature search - {:s}".format(model_name)))
        plt.clf()
