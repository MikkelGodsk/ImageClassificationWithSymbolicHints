...
def estimate_prior_class_probs(ds: torch.utils.data.Dataset, n_classes: int) -> torch.Tensor:
    """
...
class NaiveBayesFusion(LitModel):
    ...
    """
    def __init__(self, *classifiers: List[LitModel], learning_rate=0.001, adam_epsilon=1e-8,  weight_decay: List[float] = [], **kwargs):
        """
            kwargs can be used to store additional hyperparameters that needs to be logged.
        """
        super().__init__(**kwargs)
        self.save_hyperparameters()
        self._classifiers: torch.nn.ModuleList[LitModel] = torch.nn.ModuleList(classifiers)
        self._N = len(classifiers)  # Should always be the number of classifiers.
        self.log_prior_probs = None  # The pi-vector. Is not a parameter and should be set manually
    ...
    def forward_no_softmax(self, x):
        z_stack = torch.stack([clf.forward_no_softmax(x_i) for x_i, clf in zip(x, self._classifiers)])
    ...
    @property
    def top(self):
        raise NotImplemented("Not defined for naive Bayes fusion")   # Should probably inherit from elsewhere as it violates the Liskov substitution principle.
    
    def estimate_prior_class_probs(self, ds: torch.utils.data.Dataset, n_classes: Optional[int]=None, precomputed_log_probs: Optional[torch.Tensor]=None):
        """
        ...
        if n_classes is not None:
            ...
            raise ValueError("n_classes must be supplied if dataset does not contain a `.classes` property")
        ...
        self.mask = ~torch.isinf(self.log_prior_probs).view(1,-1)  # We keep everything finite.
...
class LogisticRegressionBasedFusion(LitModel):
    def __init__(self, *classifiers: List[LitModel], out_features, learning_rate=0.001, adam_epsilon=1e-8, weight_decay: float = 1e-6, **kwargs):
        """
            kwargs can be used to store additional hyperparameters that needs to be logged.
    ...
    def forward_no_top(self, x):
        with torch.no_grad():
            z_stack = torch.cat([clf.forward_no_top(x_i) for x_i, clf in zip(x, self._classifiers)], dim=1)
        return z_stack
    ...
    def _configure_optim_train(self):   # If we would like to freeze parts of the network, we should probably just return optimizers for only the non-frozen parameters here.
...
class SVMBasedFusion(LitModel):
    def __init__(self, *classifiers: List[LitModel], regularization_strength=0.003, max_iter=1000, n_jobs=1):
        super().__init__()
        ...
        self.calib_svm = CalibratedClassifierCV(   # SVM needs to be calibrated
            base_estimator=self.svm,
            n_jobs=1,
            cv='prefit'
        )
        self._cuda = False
        
    def fit_svm(self, ds: torch.utils.data.DataLoader, epochs=1, n_classes=205, verbose=True):
    ...
    def forward(self, x):
        z_stack = self.forward_no_top(x)
        probs = torch.tensor(
            self.calib_svm.predict_proba(z_stack)
        )
        if self._cuda:    probs = probs.cuda()
        return probs
...
if __name__ == '__main__':
    ...
    bimodal_val_ds: dataset.BimodalDS = dataset.BimodalDS(image_ds=val_img_ds, text_ds=val_text_ds)
    ...
    print(trainer.test(fusion_model, dataloaders=dataset.bimodal_dataloader(bimodal_val_ds)))
