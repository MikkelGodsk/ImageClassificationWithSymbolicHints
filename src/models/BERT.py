...
class LitBERTModel(LitModel):
    ...
    def __init__(
        ...
        if top_dense_layer_units != []:
            ...
            self._top.append(
                torch.nn.Linear(
                    in_features=top_dense_layer_units[-1], 
            ...
            self._top.append(
                torch.nn.Linear(
                    in_features=bert_embedding_dim, 
                    out_features=num_classes
                )
            )
...
def train_and_evaluate(params, train_ds, val_ds=None, n_epochs=20, model_version=""):
    logger = pl.loggers.TensorBoardLogger(save_dir="~/bscproj/CMPlaces", name="lightning_logs", version=model_version)
