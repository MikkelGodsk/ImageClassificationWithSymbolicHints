...
meta = scipy.io.loadmat(os.path.join(ds_dir, 'dev_kit', 'ILSVRC2012_devkit_t12', 'data', 'meta.mat'))
...
if not os.path.isdir(os.path.join(unpacked_ds_dir, 'val')):
    ...
    val_ground_truths = []
    with open(os.path.join(ds_dir, 'dev_kit', 'ILSVRC2012_devkit_t12', 'data', 'ILSVRC2012_validation_ground_truth.txt'), 'r') as f_obj:
        for target in f_obj:
    ...
    for i, img in enumerate(sorted(os.listdir(os.path.join(ds_dir, 'val_imgs')))):
        ...
        target = os.path.join(unpacked_ds_dir, 'val', synsets[val_ground_truths[i]-1], img)
...
for k, v in wiki_descriptions.items():
    ...
    combined_descriptions[k].append(wnet_descriptions[k])  # Only add the ones that exist in both
    

# Delete images without an associated Wikipedia-description.
no_wiki_text = set(wnet_descriptions.keys()).difference(set(combined_descriptions.keys()))
...
for k, v in combined_descriptions.items():
    train_ixs = np.random.choice(len(v), int(np.ceil(train_val_split*len(v))), replace=False).tolist()
