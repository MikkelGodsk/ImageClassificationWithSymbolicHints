...
class KitModel(nn.Module):
    ...
    def __init__(self, weight_file):
        ...
        self.fc8_1 = self.__dense(name = 'fc8_1', in_features = 4096, out_features = 205, bias = True)
    ...
    def forward_no_top(self, x):
        ...
        relu7           = F.relu(fc7_1)
        drop7           = F.dropout(input = relu7, p = 0.5, training = self.training)#, inplace = True)  # Inplace operations messes up gradient computations, it seems.
    ...
    def __conv(dim, name, **kwargs):
        ...
        if 'bias' in _weights_dict[name]:
            layer.state_dict()['bias'].copy_(torch.from_numpy(_weights_dict[name]['bias']).flatten())  # Added .flatten
        return layer
